...
### 3. !Смарт-контракты и протокол исполнения
    ...
    Основные принцыпы кооперации реализованы через библиотеку протокольных смарт-контрактов
...
##### **Примечание**
        Все *Coop Contracts* деплоятся только кооперациями зарегестрироваными в сети, и принадлежат компании которая их зарегестрировала.
        Исходя из этого все права на использование таких контректов находятся у кооперации владельца и могут ей распростроняться через прдоставление приватного ключа,
        по желанию кооперации, но так же эта кооперация несет полную ответственность и обязуется соблюдать оснавные принцыпы сети.
        Сеть может только проверить такой контракт на соответствие требований своего протокола, и наказть компанию согласно правилам сети.

#### 3.4 Общие правила деплоя/обновления и контроля доступа 
    - **Только кооперативы-компании** могут инициировать развертывание любого нового контракта (*Oracle Contracts* или *Oracle Contracts*) через транзакции специального типа. 
...
#### 4.1 MCP Coin
    Внутренний токен **MCP Coin** служит расчетной единицей и средством оплаты комиссий внутри сети:
        ...
        - **Ликвидность и поддержание курса:** 
            Проект поддерживает официальный пул ликвидности MCP/USDC на DEX через пулл ликвидности
        - **Целевой курс:** 
...
- **Комиссии за операции:** В сети установлены фиксированные комиссии (gas) в MCP за типовые действия:
  ...
  |-----------------------------|---------------:|------------------------------------------ |
  | Создание новой организации  |    10.0 + 0.001| Единоразовый взнос при регистрации (OrgId)|
...
### 5. !Управление сетью (Governance)
    ...
    Сеть MCP Coop Chain управляется децентрализованно посредством двух уровней DAO:
    ...
    #### 5.1 Сетевое DAO (голосование для изменений общих принципов сети) 
        ...
        касающееся всего блокчейна (например обновление протокола, внедрение новых функций):
            ...
            2. **Формирование единого голоса:** После завершения локального голосования смарт-контракт OrgId автоматически отправляет **единый голос** от организации 
            ...
            подсчитываются голоса валидаторов:
                ...
                - Так же есть минимальный порог голосования = 2/3 всех зарегестрированых компаний-кооперативов
                - Если пороги достигнуты – предложение считается принятым и исполняется (например, происходит изменение параметров сети).
                - Если нет – предложение отклоняется, с возможностю повторения голосования не раньше чем через 30 дней.
...
##### **Примечание**
	Компании так же могут высстраивать внутрение процессы через параметризирование *Proto-IP* и *Organization Contracts*, 
    не нарушая основных принципов коопераци, например: распределять роли, оплату труда, интегрировать казначейство (treasury) для управления финансами.
...
### 7. !Внутренняя структура и экономика кооперативной организации
    ...
    Эти правила закреплены в *Proto-IP* контракте для создания организации, при создании оранизации они конфигурируются параметрами 
    и подкреплены обязательствами участников через DAO.

    #### 7.1 Экономическая модель компании-кооператива 
        ...
        - **Распределение выручки:** 
            ...
            1. **Пул зарплат** – минимум 51% от всей выручки за установленый период. 
                Предназначенные для оплаты труда всех сотрудников компании. 
            2. **Пул развития/Резервный фонд** – минимум 10% от всей выручки за установленый период, так же туда попадают средства от инвесторов. 
        ...
        - **Распределение зарплат:** 
            После распределения по пуллам, из **Пула зарплат** на основе параметров высчитывается справедливая оплата труда.
            ...
            и выплачивается через смарт-контракт минимум раз в 30 дней или при вызове функции оплаты.
                ...
                2. **Ставка разряда** устанавливается при приеме на определенную позицию(например: проект-менеджер, синьер разработчик, стажер) в компании, 
                может быть изменена через DAO на основе принципов и правил сети.
                3. **Коэффициент условий**(опционально) -  если труд тяжёлый, вредный, стрессовый(например: офертайм, работа в выходной)
            Таким образом, в кооперативе устанавливается **честная система оплаты труда** гарантированая сетевым протоколом на основе смарт-контрактов. 
        ...
        - **Выплаты инвесторам:** 
            ...
            Данный подход исключает влияние инвесторов на политику компании, сохраняя полный контроль за трудовым коллективом.
            ...
            Такой механизм явлется абсолютным гарантом инвестиций и должен положительно сказаться на желании инвестировать.

 ##### **Примечание**           
    Эта экономическая модель встроена в логику блокчейна, отражая **принципы кооперативизма**, через приоритет развития и труда над капиталом. 
    ...
    #### 7.2 Политика управления и трудовые отношения 
        - **Должности и оклады:** 
        ...
        - **Изменение состава организации:**
            Прием новых содтрудников осуществляется только с полного согласия всех сотрудников коллектива
            ...
            Это обеспечивает честное распределение выручки за проделаную каждым сотрудником или стажером работу.
